 ...
 * in nums2. answer[1] is a list of all distinct integers in nums2 which are not
 ...
 * nums2[2] = 6 are not present in nums2. Therefore, answer[1] = [4,6]. Example
 * 2:
 * 
 * Input: nums1 = [1,2,3,3], nums2 = [1,1,2,2] Output: [[3],[]] Explanation: For
 * nums1, nums1[2] and nums1[3] are not present in nums2. Since nums1[2] ==
 ...
 * Accepted 153,500 Submissions 196,525
 * 
 * @author jojo Jun. 28, 2023 11:13:21 p.m.
 */
public class FindTheDifferenceInTwoArrays {
	public List<List<Integer>> findDifference(int[] nums1, int[] nums2) {
		List<List<Integer>> result = new ArrayList<>();

		result.add(getDistinctEntries(nums1, nums2));
		result.add(getDistinctEntries(nums2, nums1));

		return result;
	}

	private List<Integer> getDistinctEntries(int[] nums1, int[] nums2) {
