...
public class NumberOfIslands {
	...
	public List<Integer> numIslands2(int m, int n, int[][] positions) {
		...
		for (int[] pos : positions) {
			...
			for (int[] move : moves) {
				...
				// if the neighbor island contains a different parent island
				// then its time to join them
				if (parentIsland != currentIsland) {
					// updating the parent of current island
					islands[currentIsland] = parentIsland;
					// moving the curent island to the neighbour parent island
					// to connect the chain
					currentIsland = parentIsland;
					// decreasing the count as current island belongs a bigger
					// island
					islandCount--;
				}
			}
	...
	private int findParentIsland(int[] islands, int parentIsland) {
		// if parent is not same as self then it means it is a part of a bigger
		// island
	...
	public static void main(String[] args) {
		List<Integer> result = new NumberOfIslands().numIslands2(3, 3,
				new int[][] { { 0, 0 }, { 0, 1 }, { 1, 2 }, { 2, 1 }, { 1, 1 }, { 2, 2 } });
