...
public class FindCelebrity {
	public int findCelebrity(int n) {
		...
		// checking if anyone knows candidate
		for (int i = 1; i < n; i++) {
			if (knows(candidate, i)) {
				// potentially this is the celebrity;
				candidate = i;
			}
		}

		// checking if candidate is celebrity, this means everyone will know
		// candidate but candidate will know no one
		for (int i = 0; i < n; i++) {
