...
602
 * @author jojo
 * Apr 7, 2021  10:23:09 PM
 */
public class MinimumPathCostInAHiddenGrid {
	private Map<Pair<Integer, Integer>, Integer> graph = new HashMap<>();
	private Pair<Integer, Integer> target = null;
	private char[] steps = { 'U', 'L', 'D', 'R' };
	...
	public int findShortestPath(GridMaster master) {
		...
		while (!pQueue.isEmpty()) {
			...
			if (top[0] == target.getKey() && top[1] == target.getValue()) {
				return top[2];
			}

			for (int i = 0; i < 4; i++) {
				int[] m = moves[i];
				int x1 = top[0] + m[0], y1 = top[1] + m[1];
	...
	private void dfs(GridMaster master, int x, int y) {
		...
		for (int i = 0; i < 4; i++) {
			int[] m = moves[i];
			int x1 = x + m[0], y1 = y + m[1];
			Pair<Integer, Integer> next = new Pair(x1, y1);

			if (!graph.containsKey(next) && master.canMove(steps[i])) {
				graph.put(next, master.move(steps[i]));
				dfs(master, x1, y1);
				master.move(steps[(i + 2) % 4]);
			}
		}
	}
