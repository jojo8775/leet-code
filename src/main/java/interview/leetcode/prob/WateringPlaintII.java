...
10,089
 * @author jojo
 * Dec 12, 2021 10:28:21 PM
 */
public class WateringPlaintII {
	public int minimumRefill(int[] plants, int capacityA, int capacityB) {
        int len = plants.length, left = 0, right = len - 1, refilCount = 0;
        int curA = capacityA, curB = capacityB;
        
        while(left <= right){
            if(left != right){
                if(curA - plants[left] < 0){
                    refilCount++;
                    curA = capacityA;
                }
                ...
                if(plants[left] > curA && plants[right] > curB){
                    refilCount++;
                }
            }
